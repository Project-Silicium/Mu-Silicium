...
OPTS="$(getopt -o d:r:m:sch -l device:,release:,model:,enable-secureboot,clean,help -n 'build_uefi.sh' -- "$@")" || exit 1
eval set -- "${OPTS}"
...
python3 "$TARGET_PATH/DeviceBuild.py" "TARGET=$TARGET_BUILD_MODE" "ENABLE_SECUREBOOT=$TARGET_ENABLE_SECUREBOOT" "FD_BASE=$TARGET_FD_BASE" "FD_SIZE=$TARGET_FD_SIZE" "FD_BLOCKS=$TARGET_FD_BLOCKS" "DEVICE_MODEL=$TARGET_MODEL" || _error "\nFailed to Compile UEFI!\n"
