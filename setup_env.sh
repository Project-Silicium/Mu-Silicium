...
if [ ${PAK} = apt ]; then
    ...
    sudo apt install -y pip git mono-devel build-essential lld nuget uuid-dev nasm gcc-aarch64-linux-gnu gcc-arm-linux-gnueabihf python3 python3-distutils python3-git python3-pip gettext locales gnupg ca-certificates python3-venv git git-core clang llvm curl lld wine||_error "\nFailed to install Packages!\n"
elif [ ${PAK} = dnf ]; then
