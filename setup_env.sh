...
if [ ${PAK} = apt ]; then
    ...
    sudo apt install -y pip git mono-devel build-essential lld nuget uuid-dev iasl nasm gcc-aarch64-linux-gnu gcc-arm-linux-gnueabihf python3 python3-distutils python3-git python3-pip gettext locales gnupg ca-certificates python3-venv git git-core clang llvm curl lld wine||_error "\nFailed to install Packages!\n"
elif [ ${PAK} = dnf ]; then
    sudo dnf install -y git mono-devel nuget iasl nasm make lld gcc automake gcc-aarch64-linux-gnu arm-linux-gnueabihf-gcc python3 python3-pip gettext gnupg ca-certificates git git-core clang llvm curl lld wine||_error "\nFailed to install Packages!\n"
    ...
    yay -Sy git mono base-devel nuget uuid iasl lld nasm aarch64-linux-gnu-gcc arm-linux-gnueabihf-gcc python3 python python-distutils-extra python-git python-pip gettext gnupg ca-certificates python-virtualenv python-pipenv core-git clang llvm curl lld wine||_error "\nFailed to install Packages!\n"
else
