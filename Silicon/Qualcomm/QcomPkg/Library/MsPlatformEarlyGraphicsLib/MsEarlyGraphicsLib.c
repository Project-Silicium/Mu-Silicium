...
LABEL_33:
  ...
  DEBUG ((EFI_D_INFO, "HorizontalResolution:0x%x \n", (UINT32)(*GraphicsMode)->Info->HorizontalResolution));
  DEBUG ((EFI_D_INFO, "VerticalResolution:0x%x \n",   (UINT32)(*GraphicsMode)->Info->VerticalResolution));
  DEBUG ((EFI_D_INFO, "PixelFormat:0x%x \n",          (UINT32)(*GraphicsMode)->Info->PixelFormat));
  DEBUG ((EFI_D_INFO, "PixelsPerScanLine:0x%x \n",    (UINT32)(*GraphicsMode)->Info->PixelsPerScanLine));

  if ((*GraphicsMode)->Mode == -1) {
    DEBUG ((EFI_D_INFO, "GOP has invalid mode. Will attempt init on next call.\n"));
  } else {
    IsInitialized = 1;
  }
