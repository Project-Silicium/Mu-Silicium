  ...
  IN EFI_DEVICE_PATH_PROTOCOL    *RemainingDevicePath)
{
  ...
  //
  Status = gBS->OpenProtocol(Controller, &gQcomKeypadDeviceProtocolGuid, (VOID **)&KeypadDevice, This->DriverBindingHandle, Controller, EFI_OPEN_PROTOCOL_BY_DRIVER);
  if (EFI_ERROR(Status)) {
    return Status;
  }
  ...
  //
  gBS->CloseProtocol(Controller, &gQcomKeypadDeviceProtocolGuid, This->DriverBindingHandle, Controller);
