...
/*
Check if Volume UP is pressed to enter FrontPage.
...
/*
Check if Volume Down is pressed to enter FFU Loader.
...
GetButtonState()
{
  ...
  if (EFI_ERROR(Status)) {
    DEBUG((DEBUG_ERROR, "Failed to locate ButtonsDxe device path for button service protocol, Status = %r.\n", Status));
    goto Exit;
  }
  ...
  if (EFI_ERROR(Status)) {
    DEBUG((DEBUG_ERROR, "Failed to open ButtonsDxe protocol for button service protocol, Status = %r.\n", Status));
    goto Exit;
  }
  ...
  if (EFI_ERROR(Status)) {
    DEBUG((DEBUG_ERROR, "Failed to register volume up notification for button service protocol, Status = %r.\n", Status));
    goto Exit;
  }
  ...
  Status = SimpleEx->RegisterKeyNotify(SimpleEx, &KeyData, &KeyNotify, &NotifyHandle);
  if (EFI_ERROR(Status)) {
    DEBUG((DEBUG_ERROR, "Failed to register volume down notification for button service protocol, Status = %r.\n", Status));
    goto Exit;
  }
