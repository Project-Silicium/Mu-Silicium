...
{
  EFI_STATUS             Status;
  EFI_S2MPU09_PROTOCOL  *mS2mpu09Protocol;
  UINT32                 Value;

  // Locate S2mpu09 PMIC Protocol
  Status = gBS->LocateProtocol (&gEfiS2mpu09ProtocolGuid, NULL, (VOID *)&mS2mpu09Protocol);
  if (EFI_ERROR (Status)) {
    DEBUG ((EFI_D_ERROR, "%a: Failed to Locate S2mpu09 PMIC Protocol! Status = %r\n", __FUNCTION__, Status));
    goto Dead;
  }

  // Disable WTSR
  Status = mS2mpu09Protocol->DisableWtsr ();
  if (EFI_ERROR (Status)) {
  ...
  // Disable SMPL
  Status = mS2mpu09Protocol->DisableSmpl ();
  if (EFI_ERROR (Status)) {
