/**
  ...
  provided by TegraKeypadController.c
  ...
  IN EFI_KEY_DATA *InputData)
{
  ...
  if ((RegsiteredData->Key.ScanCode != InputData->Key.ScanCode) || (RegsiteredData->Key.UnicodeChar != InputData->Key.UnicodeChar)) {
    return FALSE;
  }
  ...
  if (RegsiteredData->KeyState.KeyShiftState != 0 && RegsiteredData->KeyState.KeyShiftState != InputData->KeyState.KeyShiftState) {
    return FALSE;
  } else if (RegsiteredData->KeyState.KeyToggleState != 0 && RegsiteredData->KeyState.KeyToggleState != InputData->KeyState.KeyToggleState) {
    return FALSE;
  }
  ...
  OUT EFI_KEY_DATA            *KeyData)
{
  ...
  if (ConsoleInDev->KeypadErr) {
    DEBUG ((EFI_D_ERROR, "An Keypad Error occured!\n"));
  ...
  IN BOOLEAN                         ExtendedVerification)
{
  ...
  if (EFI_ERROR(Status)) {
    ...
    //
    gBS->RestoreTPL(OldTpl);
    DEBUG ((EFI_D_ERROR, "Failed to Init Keypad!\n"));
    return EFI_DEVICE_ERROR;
  }
  ...
  IN VOID     *Context)
{
  ...
  if (!ConsoleIn->KeypadErr) {
    ...
    while (!IsEfikeyBufEmpty(&ConsoleIn->EfiKeyQueue)) {
      CopyMem(&KeyData, &(ConsoleIn->EfiKeyQueue.Buffer[ConsoleIn->EfiKeyQueue.Head]), sizeof(EFI_KEY_DATA));
  ...
  OUT EFI_KEY_DATA                     *KeyData)
{
  ...
  if (KeyData == NULL) {
    DEBUG ((EFI_D_ERROR, "KeyData is NULL!\n"));
    return EFI_INVALID_PARAMETER;
  }
  ...
  IN EFI_KEY_TOGGLE_STATE              *KeyToggleState)
{
  ...
  if (KeyToggleState == NULL) {
    DEBUG ((EFI_D_ERROR, "KeyToggleState is NULL!\n"));
    return EFI_INVALID_PARAMETER;
  }
  ...
  if (ConsoleInDev->KeypadErr) {
    ...
    Status = EFI_DEVICE_ERROR;
    goto Exit;
  } else if ((*KeyToggleState & EFI_TOGGLE_STATE_VALID) != EFI_TOGGLE_STATE_VALID) {
    DEBUG ((EFI_D_ERROR, "Key Toggle State is Invalid!\n"));
  ...
  } else if ((*KeyToggleState & EFI_NUM_LOCK_ACTIVE) == EFI_NUM_LOCK_ACTIVE) {
    ConsoleInDev->NumLock = TRUE;
  } else if ((*KeyToggleState & EFI_CAPS_LOCK_ACTIVE) == EFI_CAPS_LOCK_ACTIVE) {
    ConsoleInDev->CapsLock = TRUE;
  } else if ((*KeyToggleState & EFI_KEY_STATE_EXPOSED) == EFI_KEY_STATE_EXPOSED) {
  ...
  OUT VOID                            **NotifyHandle)
{
  ...
  if (KeyData == NULL || NotifyHandle == NULL || KeyNotificationFunction == NULL) {
    DEBUG ((EFI_D_ERROR, "KeyData, NotifyHandle and KeyNotificationFunction are NULL!\n"));
    return EFI_INVALID_PARAMETER;
  }
  ...
  if (NewNotify == NULL) {
    ...
    DEBUG ((EFI_D_ERROR, "Failed to Allocate Memory for Notification Function! Status = %r\n", Status));
    goto Exit;
  }
  ...
  IN VOID                              *NotificationHandle)
{
  ...
  if (NotificationHandle == NULL) {
    DEBUG ((EFI_D_ERROR, "NotificationHandle is NULL!"));
    return EFI_INVALID_PARAMETER;
  }
